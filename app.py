...
def main() -> None:
    ...
    with st.sidebar:
        ...
        date_range = st.date_input(
            "Date Range",
            value=(
                date.today() - timedelta(days=config.DEFAULT_DATE_RANGE_DAYS),
                date.today(),
            ),
        )
    ...
    # -----------------------------------------------------------------------
    total = len(filtered)
    pos = (filtered["sentiment_label"] == "POSITIVE").sum() if "sentiment_label" in filtered.columns else 0
    neg = total - pos
    avg_rating = filtered["rating"].mean() if "rating" in filtered.columns else 0
    avg_sentiment = filtered["sentiment_score"].mean() if "sentiment_score" in filtered.columns else 0

    k1, k2, k3, k4, k5 = st.columns(5)
    k1.metric("📝 Total Reviews", f"{total:,}")
    k2.metric("😊 Positive", f"{pos:,}", f"{100*pos/total:.1f}%" if total else "")
    k3.metric("😞 Negative", f"{neg:,}", f"{100*neg/total:.1f}%" if total else "")
    ...
    with col_right:
        ...
        if not issues_df.empty:
            display_issues = issues_df[
                ["rank", "keyword", "frequency", "avg_neg_strength", "priority_score"]
            ].copy()
    ...
    if st.button("Generate PDF Report"):
        with st.spinner("Building PDF…"):
            try:
                ...
                date_to = (
                    date_range[1] if len(date_range) == 2 else date.today()
                )
                with tempfile.NamedTemporaryFile(
